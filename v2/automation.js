...
export function normalizeMarketFetch(result){
 const x=result||{},price=Number(x.price);
 if(x.ok!==true||!Number.isFinite(price)||price<0)return{ok:false,reason:'invalid-market-result'};
 const checkedAt=String(x.checkedAt||'').trim(),source=String(x.source||'').trim();
 if(!checkedAt||!source)return{ok:false,reason:'missing-market-provenance'};
 return{ok:true,price,checkedAt,source};
}
export function applyMarketFetch(state,target,result){const next=clone(state),k=inventoryKey(target),i=next.marketQuotes.findIndex(x=>inventoryKey(x)===k);const normalized=normalizeMarketFetch(result);if(!normalized.ok)return{state:next,outcome:'preserved'};const quote={product:target.product,productKey:target.productKey,category:target.category,condition:target.condition,price:normalized.price,checkedAt:normalized.checkedAt,source:normalized.source,history:i>=0?[...(next.marketQuotes[i].history||[]),{price:normalized.price,checkedAt:normalized.checkedAt,source:normalized.source}]:[{price:normalized.price,checkedAt:normalized.checkedAt,source:normalized.source}]};if(i>=0)next.marketQuotes[i]=quote;else next.marketQuotes.push(quote);return{state:next,outcome:'updated'}}
