...
function decorateLedger(state){if(!location.hash.startsWith('#ledger'))return;const rows=currentLedgerRows(state),nodes=[...document.querySelectorAll('#view>.list .list-item')];nodes.forEach((node,i)=>{const item=rows[i];if(!item)return;const kind=transactionKind(item._kind),breakdown=transactionBreakdown(item),amount=node.querySelector('.amount');node.classList.remove('txn-purchase','txn-sale');if(kind)node.classList.add(`txn-${kind}`);if(!amount)return;amount.innerHTML=`<span class="transaction-label">${kind==='sale'?'売却合計':'購入合計'}</span>${breakdown.total.toLocaleString('ja-JP',{style:'currency',currency:'JPY',maximumFractionDigits:0})}${breakdown.showBreakdown?`<small class="transaction-breakdown">${breakdown.text}</small>`:''}`})}
function decorateCardDates(state){if(!location.hash.startsWith('#inventory/cards'))return;const cards=(state.cards||[]).filter(x=>Number(x.quantity)>0),nodes=[...document.querySelectorAll('#view .list-item')];nodes.forEach((node,i)=>{const item=cards[i],meta=node.querySelector('.list-meta');if(!item||!meta)return;const checked=marketCheckedDate(item);if(!checked)return;const raw=String(item.marketCheckedAt||'');if(raw)meta.textContent=meta.textContent.replace(new RegExp(`\\s*\\(${escRx(raw)}\\)`),'');if(!meta.querySelector('.market-checked-label'))meta.insertAdjacentHTML('beforeend',` <span class="market-checked-label">· 相場取得日 ${checked}</span>`)})}
