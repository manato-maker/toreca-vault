...
const hasCost=x=>x.acquisitionCost!==null&&x.acquisitionCost!==undefined&&x.acquisitionCost!=='';
export function assets(state){
 ...
 const settledSales=state.sales.filter(hasCost),pendingSales=state.sales.filter(x=>!hasCost(x));
 const realizedProfit=settledSales.reduce((s,x)=>s+saleRevenue(x)-n(x.acquisitionCost),0);
 const pendingSaleRevenue=pendingSales.reduce((s,x)=>s+saleRevenue(x),0);
 ...
 return{inventoryCost,cards,boxes,packs,total,purchases,sales,realizedProfit,pendingSales:pendingSales.length,pendingSaleRevenue,unrealizedProfit,totalProfit,netInvested,difference:unrealizedProfit};
}
