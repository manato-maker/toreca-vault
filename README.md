...
- 自動化の**唯一の編集元**: [`automation/Code.gs`](automation/Code.gs)
- Google Apps Script が現在取得する互換配布ファイル: [`automation/Code-v4.gs`](automation/Code-v4.gs)
...
自動化コードを変更するときは **`automation/Code.gs` だけを編集元として扱います**。現在の Apps Script ランナーは歴史的理由で `Code-v4.gs` URL を取得しているため、ランナー移行まではリリース時に `Code.gs` と同一内容を `Code-v4.gs` に配布します。v4を独立して編集してはいけません。
...
3. 自動化は `automation/Code.gs` を唯一の編集元にする。`Code-v4.gs` は現行ランナー向け配布物。
