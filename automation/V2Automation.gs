...
function tv2Config_(){const p=PropertiesService.getScriptProperties(),url=String(p.getProperty(TV2_SYNC_URL_PROP)||''),token=String(p.getProperty(TV2_SYNC_TOKEN_PROP)||'');if(!/^https:\/\/script\.google\.com\/macros\/s\/.+\/exec$/.test(url))throw new Error('TV2_SYNC_URL が未設定です');if(token.length<24)throw new Error('TV2_SYNC_TOKEN が未設定です');return{url,token}}
function tv2Call_(body){const c=tv2Config_(),r=UrlFetchApp.fetch(c.url,{method:'post',contentType:'text/plain;charset=utf-8',payload:JSON.stringify(Object.assign({token:c.token},body)),muteHttpExceptions:true});const j=JSON.parse(r.getContentText());if(!j.ok)throw new Error(j.error||'V2 API error');return j}
