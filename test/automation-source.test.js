...
test('自動化の編集元 Code.gs と現行GAS配布用 Code-v4.gs は完全一致', async () => {
  const [source, deployedCopy] = await Promise.all([
    read('automation/Code.gs'),
    read('automation/Code-v4.gs')
  ]);
  assert.equal(deployedCopy, source, 'Code.gs を変更したら Code-v4.gs にも同じ内容を配布してください');
});
