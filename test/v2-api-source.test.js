...
test('Apps Script verifies full payload and rollback',()=>{assert.match(src,/tv2Canonical_\(reread\).*tv2Canonical_\(next\)/);assert.match(src,/rollback verification failed/);assert.match(src,/write failed and rollback was not verified/)})
